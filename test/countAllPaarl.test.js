describe('countAllPaarl' , function(){
    it('returns the number of registration numbers in the string for Paarl' , function(){
        assert.equal(countAllPaarl('CA 182736,CY 523519,CJ 81232854678GP, 79974L ,EC5768, 23662MP'), 1);

    });

    it('returns the number of registration numbers that are from Paarl(CJ)' , function(){
        assert.equal(countAllPaarl('CA 182736,CY 523519,CJ 81232854678GP, 79974L ,EC5768, 23662MP, CJ 68845, CJ693'), 3);

    });

    it('returns the number of registration numbers that are from Paarl(CJ)' , function(){
        assert.equal(countAllPaarl('CA 182736,CY 523519, 79974L ,EC5768, 23662MP,'), 0);

    });



});